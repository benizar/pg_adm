...
COMMENT ON VIEW size_schemas IS 'Total on-disk sizes of all schemas of current database';
...
*/
CREATE OR REPLACE FUNCTION size_schema(text) RETURNS TEXT AS $$
...
COMMENT ON FUNCTION size_schema(text) IS 'Returns the pretty size of the given schema.';
...
*/
CREATE OR REPLACE FUNCTION size_schema_filter(text, text) RETURNS TEXT AS $$
...
COMMENT ON FUNCTION size_schema_filter(text, text) IS 'Returns the pretty size of the given schema. Allows you to supply a case-sensitive regular expression to only consider a subset of table names within the schema.';
